...
uint8_t temp_active_extruder;
bool temp_extruder_not_active_extruder = false;
...
void lcd_change_to_menu_change_choose_extruder();
...
static void lcd_change_to_menu_change_material_return()
{
  ...
  if (temp_extruder_not_active_extruder) {
    active_extruder = temp_active_extruder; // change active extruder back to the stored extruder if it was changed previously
    temp_extruder_not_active_extruder = false;
  }
  currentMenu = lcd_menu_print_printing;
}
...
static void lcd_menu_print_abort()
{
  ...
  if (bad_file_passcode_abort_print) {    //abort print without confirmation menu
    bad_file_passcode_abort_print = false;
    lcd_question_screen_auto_answer(lcd_menu_print_ready, abortPrint);
  } else {
    LED_GLOW();
...
static void lcd_menu_print_pause()
{
  if (card.sdprinting && !card.pause)
  {
    if (movesplanned() > 0 && commands_queued() < BUFSIZE)
    {
      ...
      sprintf_P(buffer, PSTR("M601 X20 Y5 Z%i L%i"), int(zdiff), int(END_OF_PRINT_RETRACTION));
      enquecommand(buffer);
#else               // original command, good for one extruder
