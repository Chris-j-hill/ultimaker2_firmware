...
void process_commands()
{
  ...
  else if (code_seen('M'))
  {
    switch ( (int)code_value() )
    {
      ...
      case 602:        // buffer look ahead function
      {
      /*
       * Custom code for this implementation used for dual extruder tool change 
       * 
       * The issue this is trying to solve is to avoid any leakage of the nozzle after a active extruer change
       * To change the nozzle the inactive extruder is retracted and the new active one is primed. The distance 
       * it needs to be primed is a tricky number to calculate as if it is off my a fraction of a mm it will 
       * either not be properly primed or leak material. During testing we found the print comes out better if 
       * the nozzle is parked away from the print and a temperature change is applied. This then leads to any 
       * leakage being dragged accross the print after the temperature change when the nozzle moves to resume the print
       * 
       * This function will find the next G1 code in the buffer if it exists containing and X and/or Y position and moves 
       * the nozzle there.
       */
...
//        int buffer_row = -1;
        ...
        bool y_pos_found = false;

        char pos_string[5];    //array for five elements of position
    
        float read_x_pos;
        float read_y_pos;
        
        for (i = bufindw; i<BUFSIZE;i++){   //scan end of cmdbuffer
        
          if(cmdbuffer[i][0] == 'G' && cmdbuffer[i][1] == '1'){
          
            for(j=0; j < MAX_CMD_SIZE; j++){ // once g1 found, confirm contains X or Y coordinates
               if (j =='X'){
//                 for (k=j+1; k<j+6;k++){
//                   pos_string[k-j-1] = cmdbuffer[i][k];  //copy position to array 
//                 }
//                 read_x_pos = atoi(pos_string);
                 read_x_pos = strtod(&cmdbuffer[i][j+1], NULL);
                 x_pos_found = true;
               }
               else if(j =='Y'){
//                 for (k=j+1; k<j+6;k++){
//                   pos_string[k-j-1] = cmdbuffer[i][k];  //copy position to array 
//                 }
                 read_y_pos = strtod(&cmdbuffer[i][j+1], NULL);
                 y_pos_found = true; 
               }                  
             
               if (y_pos_found && x_pos_found){
                 G1_found = true;
                 break;              
               }
            }
        ...
        if (!G1_found){                   //scan start of cmdbuffer
          for (i = 0; i<bufindw;i++){
             if(cmdbuffer[i][0] == 'G' && cmdbuffer[i][1] == '1'){
...
//                   pos_string[k-j-1] = cmdbuffer[i][k];  //copy position to array 
//                 }
//                 read_y_pos = atoi(pos_string);
                 read_y_pos = strtod(&cmdbuffer[i][j+1], NULL);
                 y_pos_found = true;                   
               }                  
             if (y_pos_found && x_pos_found){ //break row scan when x and y found
               G1_found = true;
               break; 
             }               
           }
        ...
        if (G1_found){
          if (x_pos_found)
            destination[X_AXIS] = read_x_pos;
          if (y_pos_found)
            destination[Y_AXIS] = read_y_pos;
          plan_buffer_line(destination[X_AXIS], destination[Y_AXIS], current_position[Z_AXIS]+0.2, current_position[E_AXIS], homing_feedrate[Z_AXIS] / 60, active_extruder);
        }
      }
      break;
...
#ifdef filament_sensor_pullup
  if (filament_sensor_pullup == true)
    pinMode(filament_sensor_pin, INPUT_PULLUP);   //set pin 13 as read in pin for filament sensor
  else
    pinMode(filament_sensor_pin, INPUT);

#else   //default to standard input, not pullup
  pinMode(filament_sensor_pin, INPUT);
#endif

#ifdef filament_sensor_pin_inverting
  if (filament_sensor_pin_inverting == true)
    filament_sensor_empty = HIGH;
  else
    filament_sensor_empty = LOW;
...
void check_filament_sensor() {
  ...
  if ((card.sdprinting) && (digitalRead(filament_sensor_pin) == filament_sensor_empty) ) { //check pin state, if low, switch not pressed, no filament
    ...
    int filament_change_initial_retract = 2;    //retract this much while moving
